...
SurviveXMUM 是一个数据库驱动的在线 Wiki。前端负责文档浏览、Markdown 编辑与管理界面，后端负责认证、内容持久化、投稿审核、公开版本、收藏关注、页面讨论、站内通知、图片上传，以及由已有数据派生的贡献榜与徽章。
...
- 常见业务错误码为 `400`、`401`、`403`、`404`、`405`、`409`、`500`。

> 认证失败和权限不足目前也会返回 **HTTP 200**，错误状态放在响应体 `code` 中。调用方不能只判断 HTTP 状态码。

请求本身发错时会得到明确的 4xx，而不是笼统的 500：

| 情况 | `code` | `message` |
|---|---|---|
| 访问不存在的接口 | `404` | 接口不存在 |
| 路径变量或查询参数类型不对（如 `/comments/abc`、`?page=abc`） | `400` | 参数 xxx 格式不正确 |
| 请求体不是合法 JSON，或字段类型对不上 | `400` | 请求内容格式不正确 |
| 用了该接口不支持的 HTTP 方法 | `405` | 该接口不支持 X 方法 |
| 上传文件超过 10 MB | `400` | 文件过大 |

`500` 只表示服务端确实出了未预期的异常，值得反馈给维护者；上表这些属于调用方需要自行修正的问题。
...
| 角色 | 能力 |
|---|---|
| `USER` | 投稿、草稿、收藏、关注、浏览历史、讨论、通知和反馈 |
| `ADMIN` | 普通用户能力 + 审核投稿、管理页面、处理反馈、隐藏或恢复讨论 |
...
同一规则会波及分页信封：`PageResult` 的 `total`、`page`、`size` 也是 Java `long`，因此同样以字符串返回。需要参与运算或传给 UI 组件时请显式 `Number(...)` 转换。

```json
{ "list": [], "total": "42", "page": "1", "size": "20" }
```
...
站点有几处数据看起来像“记录”，实际是每次请求现算的，没有对应的存储表：

- **贡献榜与页面贡献者**：派生自已通过的投稿与已发布的版本快照。
- **贡献者徽章**：派生自已通过投稿与可见讨论，没有授予记录；同一家族（如投稿量 1 / 10 / 50 篇）只返回已达成的最高一档。
- **标签**：只存在于 `wiki_page.tags`，随 manifest 一起下发，没有独立的标签接口。

因此这些数据会随内容审核结果变化而变化，不适合当作不可变的历史凭证缓存。
